...
The isolated P0-T20 candidate is based on protected `main@cc5a90d0190bf84e3124a7e81bbe52bc7d0820bc` and is integrated at exact candidate head `99aea33bb5159f9888d7641c47012c71a417a1b9` from final remediation code `0554747b036faba0f4185dd08ccc080fe3a1b76b`. Its Rust API accepts finite rank-two `queries`, `keys` and `values`, applies one independent existing `softmax` call per query row, and returns `[query_count, head_dim]`; its oracle and fixture preserve the same row boundary. Local `make ci` is green with 569 Python tests, 230 speculative tests and 101 Rust tests, plus 12 focused multi-query Rust tests and 66 focused oracle tests. Fresh independent Luna and GPT-5.6-Sol reviews accepted the exact integrated candidate with no findings; the exact PR checks and post-merge evidence remain publication gates. No model/LiteLLM trio, secret value, external Sonar/GitHub mutation, hardware or runtime work is permitted.
