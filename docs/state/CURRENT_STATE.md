...
Independent follow-up remains: review token identity/lifecycle without provisioning; read back Automatic Analysis enabled; perform the owner-authorized disable action; read back disabled; then authorize the first controlled CI submission. A PR trusted-data bridge remains a separate blocked design task. The already-completed `private -> public` visibility change remains a bounded, owner-authorized remediation, not an analysis-method change.
