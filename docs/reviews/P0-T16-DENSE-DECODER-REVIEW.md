...
- **Status:** candidate complete; hosted exact-head checks, final Sol gate and protected merge pending
...
- `cargo test --test dense_decoder`: **5 passed**;
- `cargo test --workspace --all-targets --locked`: **51 passed**;
...
- full `make ci`: **503 Python tests passed**, **230 reference tests passed**, with Ruff,
...
**Final verdict:** pending exact-head hosted checks and GPT-5.6-Sol gate.
