...
#[test]
fn matmul_rejects_non_finite_result() {
    let lhs = Tensor::new(vec![1, 2], vec![f32::MAX, f32::MAX]).unwrap();
    let rhs = Tensor::new(vec![2, 1], vec![f32::MAX, f32::MAX]).unwrap();
...
#[test]
fn softmax_equal_logits_are_uniform() {
    let probabilities = softmax(&[42.0, 42.0, 42.0, 42.0]).unwrap();

    for probability in probabilities {
        assert_close(probability, 0.25, f32::EPSILON);
    }
}

#[test]
fn softmax_handles_extreme_finite_range() {
    let probabilities = softmax(&[f32::MAX, -f32::MAX]).unwrap();
...
#[test]
fn rms_norm_rejects_empty_input() {
    let error = rms_norm(&[], &[], 1.0e-6).unwrap_err();
    assert_eq!(
        error,
        ReferenceError::EmptyInput {
            operation: "rms_norm"
