"""Transactional speculative-decoding state with exact commit and rollback."""
...
class StateInvariantError(ValueError):
    """Raised when a decoder-state witness violates its immutable invariants."""


class TransactionStateError(StateInvariantError):
    """Raised when a speculative round transaction is invalid or closed."""
...
class DecoderState:
    ...
    def __post_init__(self) -> None:
        try:
            ...
            validate_prefix(
                self.target_materialized,
                name="target_materialized",
            )
            validate_prefix(
                self.draft_materialized,
                name="draft_materialized",
            )
...
class RoundTransaction:
    ...
    def begin(
        ...
        return begin_round(state, proposed_tokens)
...
def begin_round(
    ...
    if not isinstance(state, DecoderState):
        raise TransactionStateError("state must be a DecoderState")
    if state.finished:
...
def _require_open(transaction: RoundTransaction) -> None:
    if not isinstance(transaction, RoundTransaction):
        raise TransactionStateError("transaction must be a RoundTransaction")
    if transaction.closed:
        raise TransactionStateError("round transaction is already closed")
...
def synchronize_pending(state: DecoderState) -> DecoderState:
    if not isinstance(state, DecoderState):
        raise StateInvariantError("state must be a DecoderState")
